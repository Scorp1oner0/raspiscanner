...
function renderNetworkCell(d) {
  ...
  if (d.vlan_id !== null && d.vlan_id !== undefined) {
    cell += ` <span class="vlan-badge" title="802.1Q VLAN tag seen on this device's traffic">VLAN ${d.vlan_id}</span>`;
  }
  // ipv6_addresses (P4): idem, mostrato solo se il device ha davvero
  // risposto al probe IPv6 (ff02::1) — quasi sempre assente su reti dove
  // IPv6 e' disabilitato lato router/host.
  if (d.ipv6_addresses && d.ipv6_addresses.length > 0) {
    const title = escapeHtml(d.ipv6_addresses.join(", "));
    cell += ` <span class="vlan-badge" title="Responded to ICMPv6 Echo Request on ff02::1: ${title}">IPv6</span>`;
  }
  return cell;
}
